...
export default function BlendyCategoriesModal({
    buttonText = "Open",
    modalTitle = "Modal",
    ...
    const handleOpenModal = () => {
        ...
        setShowModal(true);
        blendy.current?.toggle('categories');
    ...
    const handleCloseModal = () => {
        ...
        setIsAnimating(true);
        setIsCompletelyDone(false);
        blendy.current?.untoggle('categories', () => {
            setShowModal(false);
    ...
    return (
        <div className="flex items-center flex-col gap-1 justify-center">
            {showModal && createPortal(
                ...
                <button
                    style={{ width: '30px', height: '30px' }} className="rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center cursor-pointer hover:bg-zinc-500"
                    data-blendy-from="categories"
                    onClick={handleOpenModal}
                >
                    <CategoryIcon iconName={buttonText} />
                </button>
...
export function Modal({ onClose, title = "Modal", content, closeModal }: ModalProps) {
    ...
    return (
        <div className="modal z-50 border border-zinc-700" style={{ background: "var(--background-gradient)" }} data-blendy-to="categories">
            <div>
